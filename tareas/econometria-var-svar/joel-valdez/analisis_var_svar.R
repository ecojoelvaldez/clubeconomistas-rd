...
#   - Orden recursivo (base de la consigna):
#       [Crec.PIB, Inflacion, Salario real, Crec.ocupados, Desempleo]
#   - Rezago p = 2 (criterio AIC)
#   - Robustez: sistema ampliado con la TPM
#
...
#   base de la consigna: actividad -> precios -> salarios -> empleo -> desempleo
Y <- data.frame(
  crec_pib  = crec_pib,
  inflacion = inflacion,
  sal_real  = sal_real,
...
p <- 2                                             # rezago elegido (criterio AIC)
...
## ---- 12. Robustez: sistema ampliado con la TPM -----------------------------
# Se re-estima incorporando la TPM (ordenada tras la inflacion) y se comprueba
# que las respuestas del empleo y el desempleo a un choque de actividad
# conservan signo, magnitud y persistencia.
tpm <- panel$tpm[-1]
Y6 <- ts(data.frame(crec_pib, inflacion, tpm, sal_real, crec_ocu, desempleo),
